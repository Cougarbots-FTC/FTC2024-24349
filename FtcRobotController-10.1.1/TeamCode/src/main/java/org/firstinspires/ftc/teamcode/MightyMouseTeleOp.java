...
public class MightyMouseTeleOp extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        pidf = new PIDFArm(robot.armRotator,0.007,0,0.0005,0,3);
        pidf2 = new PIDFArm(robot.armRotator2,0.007,0,0.0005,0,2);
        ...
        while (opModeIsActive()) {
            ...
            int num = 100*(int)(armRotatorUp/0.01);
            if (armRotatorUp > 0.1) {
                pidf.setSetpoint(pidf.getCurrentPosition()+num); // Set encoder position to 1000
                pidf2.setSetpoint(pidf2.getCurrentPosition()+num); // Set encoder position to 2000
            }
            // this is for lowering the arm
            double armRotatorDown = gamepad2.right_trigger;
            int val = 100*(int)(armRotatorDown/0.01);

            //int val = Math.round(armRotatorDown)*10;
            if (armRotatorDown > 0.1) {
                pidf.setSetpoint(pidf.getCurrentPosition()-val); // Set encoder position to 1000
                pidf2.setSetpoint(pidf2.getCurrentPosition()-val); // Set encoder position to 2000
            }
